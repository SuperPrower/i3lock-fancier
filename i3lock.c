 ...
 *
 * i3lock-color by eBrnd and chrjguill
 * i3lock keyboard layout and caps lock indicator by cac03
 * i3lock-fancier by SuperPrower
 *
 */
...
/* The display, to determine Caps Lock state and keyboard layout */
Display* _display;
...
static void handle_key_press(xcb_key_press_event_t *event) {
	...
	if (xkb_compose_state && xkb_compose_state_feed(xkb_compose_state, ksym) == XKB_COMPOSE_FEED_ACCEPTED) {
		...
		case XKB_COMPOSE_COMPOSED:
			/*
			 * xkb_compose_state_get_utf8 doesn't include
			 * the terminating byte in the return value
			 * as xkb_keysym_to_utf8 does. Adding one makes
			 * the variable n consistent.
			 */
	...
	while ((o = getopt_long(argc, argv, optstring, longopts, &longoptind)) != -1) {
		switch (o) {
			...
			case 'c':
				...
				break;
			default:
				errx(EXIT_FAILURE, "Syntax: i3lock [-v] [-n] [-b]"
						  " [-c config.ini]\n"
